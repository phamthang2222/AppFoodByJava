...
public class OutstandingFoodActivity extends BaseActivity {
    ...
    ActivityOutstandingFoodBinding binding;
    private RecyclerView.Adapter adapter;
    private String searchText;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityOutstandingFoodBinding.inflate(getLayoutInflater());
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    for(DataSnapshot item: snapshot.getChildren()){
                        ...
                        if(food.getBestFood()== true){
                            listFoods.add(food);
                        }
                    }
                }
                if(listFoods.size() > 0){
                    binding.rcvFoods.setLayoutManager(new GridLayoutManager(OutstandingFoodActivity.this,1));
